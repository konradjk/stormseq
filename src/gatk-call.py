...
if options.call_all_dbsnp:
  ...
  command = 'java -Xmx6500m -jar %s -T UnifiedGenotyper --dbsnp %s --output_mode EMIT_ALL_CONFIDENT_SITES --interval_set_rule INTERSECTION -L %s -L %s -R %s -I %s -o %s %s' % (gatk_binary, dbsnp, dbsnp_chr, chromosome, ref, recal_bam, dbsnp_vcf, gatk_options)
  run_gatk_commands(command)
