...
def start_sample(index, sample, single):
    ...
    try:
        command = 'sudo starcluster terminate -c -f stormseq_%s' % sample
        ...
        try:
            for i in range(3):
                try:
                    ...
                    if e.output.find('The requested Availability Zone is currently constrained') > -1 or e.output.find('not supported in your requested Availability Zone') > -1:
                        raise
                    elif i == 2:
            ...
            kill_cmd = 'sudo starcluster terminate -c --force stormseq_%s' % (sample)
            ...
            for i in range(3):
                try:
                    ...
                    if stdout.find('The cluster is now ready to use.') == -1:
                        kill_cmd = 'starcluster terminate -c --force stormseq_%s' % (sample)
