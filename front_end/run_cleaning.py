...
try:
    ...
    call_qsub = 'qsub -hold_jid %(hold)s -e /mydata/logs/ -o /mydata/logs/ -b y -cwd -R y -l virtual_free=6400M python %(program)s-call.py %(lite)s --bam=/mydata/%(bam)s --dbsnp=%(dbsnp)s --reference=%(reference)s --chromosome=%(chromosome)s %(intervals_call)s %(indels)s'
    ...
    if 'type_exome_' in parameters['data_type']:
        inputs['intervals'] = '--intervals=/data/intervals/' + parameters['data_type'].replace('type_exome_', '') + '.interval_list'
    ...
    inputs['intervals_call'] = inputs['intervals']
    if inputs['program'] == 'gatk':
        ...
        if inputs['intervals_call'] != '': inputs['intervals'] += '.bed'
